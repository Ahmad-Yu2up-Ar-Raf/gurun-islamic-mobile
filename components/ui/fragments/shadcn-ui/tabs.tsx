...
function Tabs({
  ...
  return (
    <TabsPrimitive.Root className={cn('overflow-hidde flex flex-col', className)} {...props} />
  );
}
...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      className={cn(
        'flex h-fit flex-row items-center justify-center gap-2 rounded-2xl p-1',
...
function TabsTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        'text-foreground dark:text-muted-foreground text-sm font-medium py-3 ',

        value === props.value && '    '
      )}>
      <TabsPrimitive.Trigger
        className={cn(
          'group flex h-[calc(100%-1px)] flex-row items-center justify-center gap-1.5 border-b-2 border-border px-2 py-0.5 shadow-none shadow-black/5',
          ...
          props.value === value && 'border-secondary',
          className
        )}
        {...props}
      />
